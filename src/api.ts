...
import { nativeTerminalTransport, type TransportState } from "./terminalTransport";
...
export const api = {
  ...
  async saveProfile(draft: MachineDraft): Promise<MachineProfile> {
    ...
    const saved: MachineProfile = { ...draft, id: draft.id || crypto.randomUUID(), privateKeyPath: draft.privateKeyPath || undefined, hasSavedPassword: draft.savePassword ? !!draft.password || old?.hasSavedPassword || false : false, hasSavedPassphrase: draft.savePassphrase ? !!draft.passphrase || old?.hasSavedPassphrase || false : false, createdAt: old?.createdAt || now, updatedAt: now };
  ...
  async saveSettings(settings: AppSettings): Promise<AppSettings> {
    ...
    if (!native()) localStorage.setItem(SETTINGS, JSON.stringify(saved));
    // These two stay unconditional: TerminalPane and the transport read them
    // synchronously from localStorage in native mode too.
  ...
  async connect(profileId: string, password?: string, passphrase?: string): Promise<SessionSummary> {
    ...
    const profile = read<MachineProfile[]>(PROFILES, []).find((x) => x.id === profileId); if (!profile) throw new Error("Machine not found"); const session = { id: crypto.randomUUID(), profileId, profileName: profile.name, status: "connecting" as const };
  ...
  windowLabel(): string { return native() ? getCurrentWebviewWindow().label : "main"; },
  async windowPositioningReliable(): Promise<boolean> { return native() ? invoke("window_positioning_reliable") : true; },
  async emitAppEvent(name: string, payload?: unknown) { if (native()) return tauriEmit(name, payload); emit(name, payload); },
  onAppEvent<T>(name: string, cb: (payload: T) => void): Promise<UnlistenFn | (() => void)> { if (native()) return listen(name, (e) => cb(e.payload as T)); const fn = (e: Event) => cb((e as CustomEvent).detail as T); window.addEventListener(name, fn); return Promise.resolve(() => window.removeEventListener(name, fn)); },
  onCloseRequested(handler: (event: { preventDefault: () => void }) => void | Promise<void>): Promise<UnlistenFn | (() => void)> { if (native()) return getCurrentWindow().onCloseRequested(handler); return Promise.resolve(() => {}); },
  onTransportState(cb: (state: TransportState) => void): () => void { return native() ? nativeTerminalTransport.onState(cb) : () => {}; },
};
