...
class NativeTerminalTransport {
  ...
  private async open(): Promise<void> {
    this.info ??= await invoke<TransportInfo>("terminal_transport_info");
    if (this.info.protocolVersion !== 1) throw new Error("Unsupported terminal transport version");
    await new Promise<void>((resolve, reject) => {
      const socket = new WebSocket(`ws://127.0.0.1:${this.info!.port}`);
      socket.binaryType = "arraybuffer";
      socket.onopen = () => { socket.send(this.info!.token); this.socket = socket; this.attempts = 0; resolve(); };
  ...
  private reconnect() {
    if (this.retry || this.listeners.size === 0 || this.attempts >= 5) return;
  ...
  private send(frame: Uint8Array) {
    ...
    else void this.connect().then(() => this.socket?.send(frame));
  }

  subscribe(id: string, listener: OutputListener): () => void {
    const listeners = this.listeners.get(id) ?? new Set<OutputListener>();
    listeners.add(listener); this.listeners.set(id, listeners); void this.connect();
